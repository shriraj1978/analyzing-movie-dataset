...
def duplicate_and_unique_movies(dataset, index_):
    ...
    movies_names = [dataset[i][index_] for i in range(len(dataset))]

    movies_name_count = [[x,movies_names.count(x)] for x in set(movies_names)]

    for i in range(len(movies_name_count)):
        if movies_name_count[i][1] > 1:
            duplicates.append(movies_name_count[i])
        else: 
            uniques.append(movies_name_count[i])  
    i += i

    print("Duplicates Movies: ",len(duplicates))
    print("Uniques Movies: ",len(uniques))
...
def movies_lang(dataset, index_, lang_):
    ...
    movie_name = [dataset[i][13] for i in range(len(dataset))]
...
def rate_bucket(dataset, rate_low, rate_high):
    ...
    for i in range(len(dataset)):
        if float(rate_low) <= float(dataset[i][11]) <= float(rate_high):
...
movie_name = [movies[i][13] for i in range(len(movies))]
reviews = [movies[i][12] for i in range(len(movies))]

for i in range(len(movie_name)):
    review_max[movie_name[i]] = reviews[i]
    i += 1
...
for movie in movies:
    ...
    if (review_max[name] == reviews) and name not in duplicate:
